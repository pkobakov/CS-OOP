...
namespace OnlineShop.Core
{
    ...
    {
        private readonly ICollection<IComputer> _computers;
        ...
        {
            IComputer computer = _computers.OrderByDescending(c => c.OverallPerformance).Where(c => c.Price <= budget).FirstOrDefault();
            if (computer == null)
            {
        ...
        => _computers.Any(c => c.Id == computerId) ? _computers.FirstOrDefault(c => c.Id == computerId) : throw new ArgumentException(ExceptionMessages.NotExistingComputerId);
    }
}
